...
const fetchBlogsUsername = "haleyspappa";
const fetchBlogsPassword = "unifacts";
...
async function fetchBlogs() {
  const credentials = btoa(`${fetchBlogsUsername}:${fetchBlogsPassword}`);
    ...
    json.forEach(function (blogs) {
      if (blogs.categories[0] == 26) {
        blogsDiv.innerHTML += `
          <a href="/html/blog.html?id=${blogs.id}" class="blogDivs">
              <h2>${blogs.title.rendered}</h2>
              <div class="blogsImgsContainer">
                  <img class="blogsImgs" src="${blogs._embedded["wp:featuredmedia"][0].source_url}" alt="${blogs._embedded["wp:featuredmedia"][0].alt_text}"/>
              </div>
              <hr class="hr">
          </a>
        `;
